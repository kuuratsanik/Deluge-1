...
class OptionsDialog():
    ...
    def show(self, options={}, watchdir_id=None):
        ...
        self.main_builder.add_from_file(get_resource('autoadd_options.ui'))
        ...
        self.dialog = self.main_builder.get_object('options_dialog')
        self.dialog.set_transient_for(component.get('Preferences').pref_dialog)

        if watchdir_id:
            # We have an existing watchdir_id, we are editing
            self.main_builder.get_object('opts_add_button').hide()
            self.main_builder.get_object('opts_apply_button').show()
            self.watchdir_id = watchdir_id
        else:
            # We don't have an id, adding
            self.main_builder.get_object('opts_add_button').show()
            self.main_builder.get_object('opts_apply_button').hide()
    ...
    def load_options(self, options):
        self.main_builder.get_object('enabled').set_active(options.get('enabled', True))
        self.main_builder.get_object('append_extension_toggle').set_active(
            options.get('append_extension_toggle', False)
        )
        self.main_builder.get_object('append_extension').set_text(
            options.get('append_extension', '.added')
        )
        self.main_builder.get_object('download_location_toggle').set_active(
            options.get('download_location_toggle', False)
        )
        self.main_builder.get_object('copy_torrent_toggle').set_active(
            options.get('copy_torrent_toggle', False)
        )
        self.main_builder.get_object('delete_copy_torrent_toggle').set_active(
            options.get('delete_copy_torrent_toggle', False)
        )
        self.main_builder.get_object('seed_mode').set_active(
            options.get('seed_mode', False)
        )
        self.accounts.clear()
        self.labels.clear()
        combobox = self.main_builder.get_object('OwnerCombobox')
        ...
        for id in self.spin_ids + self.spin_int_ids:
            self.main_builder.get_object(id).set_value(options.get(id, 0))
            self.main_builder.get_object(id + '_toggle').set_active(options.get(id + '_toggle', False))
        for id in self.chk_ids:
            self.main_builder.get_object(id).set_active(bool(options.get(id, True)))
            self.main_builder.get_object(id + '_toggle').set_active(options.get(id + '_toggle', False))
        if not options.get('add_paused', True):
            self.main_builder.get_object('isnt_add_paused').set_active(True)
        if not options.get('queue_to_top', True):
            self.main_builder.get_object('isnt_queue_to_top').set_active(True)
        if not options.get('auto_managed', True):
            self.main_builder.get_object('isnt_auto_managed').set_active(True)
        for field in ['move_completed_path', 'path', 'download_location',
                      'copy_torrent']:
            if client.is_localhost():
                self.main_builder.get_object(field + '_chooser').set_current_folder(
                    options.get(field, os.path.expanduser('~'))
                )
                self.main_builder.get_object(field + '_chooser').show()
                self.main_builder.get_object(field + '_entry').hide()
            else:
                self.main_builder.get_object(field + '_entry').set_text(
                    options.get(field, '')
                )
                self.main_builder.get_object(field + '_entry').show()
                self.main_builder.get_object(field + '_chooser').hide()
        self.set_sensitive()

        def on_core_config(config):
            if client.is_localhost():
                self.main_builder.get_object('download_location_chooser').set_current_folder(
                    options.get('download_location', config['download_location'])
                )
                if options.get('move_completed_toggle', config['move_completed']):
                    self.main_builder.get_object('move_completed_toggle').set_active(True)
                    self.main_builder.get_object('move_completed_path_chooser').set_current_folder(
                        options.get('move_completed_path', config['move_completed_path'])
                    )
                if options.get('copy_torrent_toggle', config['copy_torrent_file']):
                    self.main_builder.get_object('copy_torrent_toggle').set_active(True)
                    self.main_builder.get_object('copy_torrent_chooser').set_current_folder(
                        options.get('copy_torrent', config['torrentfiles_location'])
                    )
            else:
                self.main_builder.get_object('download_location_entry').set_text(
                    options.get('download_location', config['download_location'])
                )
                if options.get('move_completed_toggle', config['move_completed']):
                    self.main_builder.get_object('move_completed_toggle').set_active(
                        options.get('move_completed_toggle', False)
                    )
                    self.main_builder.get_object('move_completed_path_entry').set_text(
                        options.get('move_completed_path', config['move_completed_path'])
                    )
                if options.get('copy_torrent_toggle', config['copy_torrent_file']):
                    self.main_builder.get_object('copy_torrent_toggle').set_active(True)
                    self.main_builder.get_object('copy_torrent_entry').set_text(
                        options.get('copy_torrent', config['torrentfiles_location'])
                    )

            if options.get('delete_copy_torrent_toggle', config['del_copy_torrent_file']):
                self.main_builder.get_object('delete_copy_torrent_toggle').set_active(True)
        ...
        def on_accounts(accounts, owner):
            ...
            for account in accounts:
                ...
                if account['username'] == owner:
                    selected_iter = iter
            self.main_builder.get_object('OwnerCombobox').set_active_iter(selected_iter)
        ...
        def on_accounts_failure(failure):
            ...
            self.main_builder.get_object('OwnerCombobox').set_active(0)
            self.main_builder.get_object('OwnerCombobox').set_sensitive(False)
        ...
        def on_get_enabled_plugins(result):
            if 'Label' in result:
                self.main_builder.get_object('label_frame').show()
                client.label.get_labels().addCallback(on_labels).addErrback(on_failure)
            else:
                self.main_builder.get_object('label_frame').hide()
                self.main_builder.get_object('label_toggle').set_active(False)
    ...
    def on_toggle_toggled(self, tb):
        ...
        if toggle == 'download_location':
            ...
            self.main_builder.get_object('download_location_entry').set_sensitive(isactive)
        elif toggle == 'append_extension':
            self.main_builder.get_object('append_extension').set_sensitive(isactive)
        elif toggle == 'copy_torrent':
            self.main_builder.get_object('copy_torrent_entry').set_sensitive(isactive)
            self.main_builder.get_object('copy_torrent_chooser').set_sensitive(isactive)
            self.main_builder.get_object('delete_copy_torrent_toggle').set_sensitive(isactive)
        elif toggle == 'move_completed':
            self.main_builder.get_object('move_completed_path_chooser').set_sensitive(isactive)
            self.main_builder.get_object('move_completed_path_entry').set_sensitive(isactive)
            self.main_builder.get_object('move_completed').set_active(isactive)
        elif toggle == 'label':
            self.main_builder.get_object('label').set_sensitive(isactive)
        elif toggle == 'max_download_speed':
            self.main_builder.get_object('max_download_speed').set_sensitive(isactive)
        elif toggle == 'max_upload_speed':
            self.main_builder.get_object('max_upload_speed').set_sensitive(isactive)
        elif toggle == 'max_connections':
            self.main_builder.get_object('max_connections').set_sensitive(isactive)
        elif toggle == 'max_upload_slots':
            self.main_builder.get_object('max_upload_slots').set_sensitive(isactive)
        elif toggle == 'add_paused':
            self.main_builder.get_object('add_paused').set_sensitive(isactive)
            self.main_builder.get_object('isnt_add_paused').set_sensitive(isactive)
        elif toggle == 'queue_to_top':
            self.main_builder.get_object('queue_to_top').set_sensitive(isactive)
            self.main_builder.get_object('isnt_queue_to_top').set_sensitive(isactive)
        elif toggle == 'auto_managed':
            self.main_builder.get_object('auto_managed').set_sensitive(isactive)
            self.main_builder.get_object('isnt_auto_managed').set_sensitive(isactive)
        elif toggle == 'stop_at_ratio':
            self.main_builder.get_object('remove_at_ratio_toggle').set_active(isactive)
            self.main_builder.get_object('stop_ratio_toggle').set_active(isactive)
            self.main_builder.get_object('stop_at_ratio').set_active(isactive)
            self.main_builder.get_object('stop_ratio').set_sensitive(isactive)
            self.main_builder.get_object('remove_at_ratio').set_sensitive(isactive)
    ...
    def generate_opts(self):
        ...
        options = {}
        options['enabled'] = self.main_builder.get_object('enabled').get_active()
        if client.is_localhost():
            options['path'] = self.main_builder.get_object('path_chooser').get_filename()
            options['download_location'] = self.main_builder.get_object(
                'download_location_chooser').get_filename()
            options['move_completed_path'] = self.main_builder.get_object(
            ...
            options['path'] = self.main_builder.get_object('path_entry').get_text()
            options['download_location'] = self.main_builder.get_object(
        ...
        #  FIXME AttributeError: 'CellView' object has no attribute 'get_text'
        ...
        for key in ['append_extension_toggle', 'download_location_toggle',
            ...
            options[key] = self.main_builder.get_object(key).get_active()

        for id in self.spin_ids:
            options[id] = self.main_builder.get_object(id).get_value()
            options[id + '_toggle'] = self.main_builder.get_object(id + '_toggle').get_active()
...
class GtkUI(GtkPluginBase):
    def enable(self):
        ...
        vbox = self.main_builder.get_object('watchdirs_vbox')
        ...
        component.get('Preferences').add_page(
            _('AutoAdd'), self.main_builder.get_object('prefs_box')
        )
    ...
    def on_listitem_activated(self, treeview):
        tree, tree_id = self.treeView.get_selection().get_selected()
        if tree_id:
            self.main_builder.get_object('edit_button').set_sensitive(True)
            self.main_builder.get_object('remove_button').set_sensitive(True)
        else:
            self.main_builder.get_object('edit_button').set_sensitive(False)
            self.main_builder.get_object('remove_button').set_sensitive(False)
    ...
    def cb_get_config(self, watchdirs):
        ...
        if self.main_builder.get_object('edit_button'):
